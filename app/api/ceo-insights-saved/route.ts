...
export async function GET() {
  try {
    ...
    return NextResponse.json({ saved: { savedAt: row[0] ?? '', text: row[1] } })
  } catch (e) {
  ...
  try {
    ...
    await sheets.spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        ...
        data: [
          { range: `${SHEET_NAME}!A1:B1`, values: [['Guardado el', 'Informe (texto)']] },
          { range: `${SHEET_NAME}!A2:B2`, values: [[savedAt, text]] },
        ],
      },
    })
