...
export async function POST(req: Request) {
  try {
    const { personas, filters } = await req.json()
    const sheets = google.sheets({ version: 'v4', auth: getAuth() })
    await ensureSheetExists(sheets)
        ...
        data: [
          {
            ...
            values: [['Guardado el', 'User Personas (JSON)', 'Filtros (JSON)']],
          },
          {
            range: `${SHEET_NAME}!A2:C2`,
            values: [[savedAt, JSON.stringify(personas), JSON.stringify(filters ?? {})]],
          },
        ],
      },
    })
