...
import { normalizeSitemapPersist, createEmptySitemapPersist } from '@/lib/sitemap-moa-types'

export const dynamic = 'force-dynamic'

const SHEET_NAME = 'sitemap_moa'
...
export async function GET() {
  try {
    ...
    const data = normalizeSitemapPersist(JSON.parse(row[1]))
...
export async function POST(req: Request) {
  try {
    ...
    const data = normalizeSitemapPersist(body.data ?? createEmptySitemapPersist())
    const sheets = google.sheets({ version: 'v4', auth: getAuth() })
    await ensureSheetExists(sheets)
    ...
    await sheets.spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        ...
        data: [
          { range: `${SHEET_NAME}!A1:B1`, values: [['Guardado el', 'Sitemap (JSON)']] },
