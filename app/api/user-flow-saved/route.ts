...
export async function POST(req: Request) {
  try {
    const { flows, filters } = await req.json()
    const sheets = google.sheets({ version: 'v4', auth: getAuth() })
    await ensureSheetExists(sheets)
        ...
        data: [
          {
            ...
            values: [['Guardado el', 'User Flow (JSON)', 'Filtros (JSON)']],
          },
          {
            range: `${SHEET_NAME}!A2:C2`,
            values: [[savedAt, JSON.stringify(flows), JSON.stringify(filters ?? {})]],
          },
        ],
      },
    })
