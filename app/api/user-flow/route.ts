...
### Campo opcional de retroceso (retornoTipo / retornoA / retornoLabel)

Usalo SOLO cuando la persona deba volver a repetir un paso ANTERIOR (ej: "Si no encuentra hueco libre, vuelve a consultar horarios"). Reglas estrictas:
- "retornoA" debe ser el tituloBolita de un paso con orden MENOR al paso actual (nunca el siguiente ni el mismo).
- Nunca uses retornoA para avanzar: si la persona continua al paso siguiente, eso es un clicEntrePasos, no un retorno.
- Usa retornoA con moderacion: maximo 1-2 veces en todo el flujo, solo cuando el journey muestre una reincidencia real.
- Si no hay retroceso real, pon retornoTipo=null, retornoA=null, retornoLabel=null en todos los pasos.
